...
for key, val in counts.items():
    newTuple = (
        val,
        key,
    )  # value becomes first element of each tuple, so that we can later sort by value
    lst.append(newTuple)

lst = sorted(
    lst, reverse=True
)  # sorting our list by value and reversing the order so that biggest values come first
...
reversedSortedTuple = sorted(
    [(v, k) for k, v in counts.items()], reverse=True
